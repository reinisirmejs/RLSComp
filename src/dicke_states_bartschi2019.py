...
def test_circuit_qasm(qc, figsize=(12, 4)):
    ...
    job_sim = backend.run(qc_compiled, shots=1024, seed_simulator=42)
...
def gate_i(n, draw=False):
    ...
    cry = RYGate(theta).control(1)  # Single control qubit
    qc_i.append(cry, [1, 0])        # [control, target] POSSIBLY REVERSED
    ...
    # Draw if needed
    if draw:
        qc_i.draw("mpl")  # or use matplotlib viewer if needed
...
def first_block(n, k, l, draw=False):
    ...
    qc_first_block = QuantumCircuit(n)
    ...
    if n_first != 0:
        idxs_scs = idxs_scs[n_first:]  # skip these qubits for SCS
        qc_first_block.id(list(range(n_first)))  # identity to first block

    # Apply identity to last qubits (if needed)
    if n_last != 0:
        idxs_scs = idxs_scs[:-n_last]  # skip these qubits for SCS
        qc_first_block.append(gate_scs_nk(l, k), idxs_scs)
        qc_first_block.id(list(range(n - n_last, n)))  # identity to last block
    else:
...
def second_block(n, k, l, draw=False):    
    ...
    qc_second_block = QuantumCircuit(n)
    ...
    if n_last !=0:
        ...
        qc_second_block.id(range(n-n_last, n))
        #qc_second_block.append(gate_i(n_last), [n_last-1, n_last])
